...
object CommandSecurityFilter {
    ...
    // Blacklist: Commands that cause catastrophic, irreversible damage
    private val BLACKLIST_PATTERNS = listOf(
        Pattern.compile("rm\\s+(-[a-zA-Z]*r[a-zA-Z]*f|-[a-zA-Z]*f[a-zA-Z]*r)\\s+/", Pattern.CASE_INSENSITIVE),
    ...
    // High Risk: Data deletion, Telephony, SMS, Camera, System Reboot, Process Killing
    ...
    private val MEDIUM_RISK_PATTERNS = listOf(
        ...
        Pair(Pattern.compile("\\b(python|python3|bash|sh|node)\\b"), "Ausführung eines Skripts"),
